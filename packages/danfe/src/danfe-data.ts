...
export function extractDanfeData(xml: string): DanfeData {
  const parsed = parser.parse<any>(xml);

  // Navegar: nfeProc > NFe > infNFe  ou  NFe > infNFe
  const nfeProc = parsed.nfeProc;
  const nfeArr = nfeProc ? nfeProc.NFe : parsed.NFe;
  const nfe = Array.isArray(nfeArr) ? nfeArr[0] : nfeArr;
  const infNFe = nfe?.infNFe || nfe;

  // Protocolo
  const protArr = nfeProc?.protNFe;
  const prot = Array.isArray(protArr) ? protArr[0] : protArr;
  const infProt = prot?.infProt || prot || {};

  // Chave de acesso do Id
  const id = infNFe?.['@_Id'] || '';
  ...
  // Items
  const detArr = infNFe?.det || [];
  const dets = Array.isArray(detArr) ? detArr : [detArr];

  const itens = dets.map((d: any) => {
    const prod = d?.prod || {};
    const imposto = d?.imposto || {};
    const icms = imposto?.ICMS || {};
    const icmsObj = Object.values(icms)[0] as any || {};
  ...
  // Duplicatas
  const dupArr = cobr?.dup || [];
  const dups = Array.isArray(dupArr) ? dupArr : (dupArr ? [dupArr] : []);

  // Pagamentos
  const detPagArr = pag?.detPag || [];
  const detPags = Array.isArray(detPagArr) ? detPagArr : [detPagArr];

  // Volumes
  const volArr = transp?.vol || [];
  const vols = Array.isArray(volArr) ? volArr : (volArr ? [volArr] : []);
